...
const conflictingKeys = (...objs) => _.intersection(...objs.map((obj) => Object.keys(obj)))
...
module.exports = (opts) => {
  ...
  const bracketData = {
    ...
    scoreAll: (master) => _.orderBy(new Scorer({sport, year}).score(opts.scoring, {entry: entries, master: master || pickedMaster}), 'score', 'desc')
  }
