...
var getResult = {
  ...
  diff: function (bd, options) {
    ...
    if (
      (options.bonusStatus === "incorrect" ||
        options.bonusStatus === "correct") &&
      options.diff
    ) {
      options.diff[options.regionId].rounds[options.roundIndex][
        options.gameIndex
      ].winsInCorrect = options.bonusStatus === "correct"
    }

    if (options.diff && options.masterGame) {
      options.diff[options.regionId].rounds[options.roundIndex][
        options.gameIndex
      ].shouldBe = options.masterGame
    }
    ...
    if (options.status === "incorrect") {
      ...
      options.status === "unplayed" &&
      options.eliminated.contains(options.game)
    ) {
      if (options.diff) {
        options.diff[options.regionId].rounds[options.roundIndex][
          options.gameIndex
      ...
      options.status === "unplayed" &&
      options.pprMethods.length &&
      options.getScoreResult
    ) {
      _each(options.pprMethods, function (pprMethod) {
        options.results[pprMethod] += self.totalScore(bd, {
          ...
          bonusStatus:
            options.bonusStatus === "incorrect" ||
            options.bonusStatus === "unused" ||
            options.eliminated.contains(options.game) ||
            options.eliminated.contains(options.opponentGame)
              ? "incorrect"
              : "correct",
  ...
  bonus: function (options) {
    options.bonus[options.roundIndex] +=
...
var initialValues = {
  rounds: function (bd) {
    var count = roundCount(
      bd.constants.TEAMS_PER_REGION * bd.constants.REGION_COUNT
    )
    var rounds = []
...
function Scorer(options) {
  ...
  _each(
    _extend(this.bracketData.scoring, options.scoring || {}),
    function (system, key) {
  ...
  this.entryValidator = new BracketValidator({
    year: options.year,
    sport: options.sport,
  })
  this.masterValidator = new BracketValidator({
    year: options.year,
    sport: options.sport,
  })
...
Scorer.prototype.score = function (methods, options) {
  ...
  if (
    _isPlainObject(methods) &&
    (methods.entry || methods.master) &&
    !options
  ) {
    options = methods
  }
    ...
    _each(region.rounds, function (round, roundIndex) {
      ...
      if (getScoreResult || getDiffResult) {
        _each(round, function (game, gameIndex) {
          ...
          var hasPPR = _some(methods, function (m) {
            return m.indexOf("PPR") > -1
          })
          // But we should only call it once
          if (hasDiff || hasPPR) {
            callableMethods = _uniq(
              _map(methods, function (m) {
