var DATA_DIR = './data/ncaa-mens-basketball/',
    LIB_DIR = './lib/',
    data = require(DATA_DIR + 'data'),
    order = require(DATA_DIR + 'order'),
    finder = require(LIB_DIR + 'finder'),
    generator = require(LIB_DIR + 'generator'),
    validator = require(LIB_DIR + 'validator'),
    quickCheck = require(LIB_DIR + 'quickCheck'),
    _ = require('lodash');

module.exports = function(options) {
  options = _.defaults(options || {}, {
    year: '',
    props: 'data order finder generator validator quickCheck'.split(' ')
  });
  var thisData = data(options.year),
      thisOrder = order(options.year),
      returnVal = _.pick({
        data: thisData,
        order: thisOrder,
        finder: finder,
        generator: generator({data: thisData, order: thisOrder}),
        validator: validator({data: thisData}),
        quickCheck: quickCheck({data: thisData})
      }, options.props);

  return returnVal;
};