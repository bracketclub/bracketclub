...
var wrapError = function () {
  return {
    error: true,
    result: new Error(
      _map(_toArray(arguments), function (arg) {
...
Validator.prototype.validate = function (flatBracket) {
  ...
  result = findResult(
    this.validateFinal(
      _find(result, function (item) {
        return item.id === self.bracketData.constants.FINAL_ID
      }),
      result
    )
  )
...
Validator.prototype.expandFlatBracket = function (flat, allowEmpty) {
  if (
    !allowEmpty &&
    flat.indexOf(this.bracketData.constants.UNPICKED_MATCH) > -1
  ) {
  ...
  try {
    return wrapSuccess(
      JSON.parse(flat.replace(this.bracketData.regex, replacer))
    )
  } catch (e) {
...
Validator.prototype.hasNecessaryKeys = function (obj) {
  ...
  var hasAllKeys = !!(
    this.bracketData.constants.ALL_IDS.length === hasKeys.length &&
    _difference(this.bracketData.constants.ALL_IDS, hasKeys).length === 0
  )
  ...
  return wrapError(
    ...
    _difference(this.bracketData.constants.ALL_IDS, hasKeys).join(",")
  )
}
...
Validator.prototype.decorateValidated = function (bracket) {
  ...
  _each(bracket, function (region) {
    decorated[region.id] = _extend(
      {},
      region,
      self.bracketData.bracket.regions[region.id] ||
        self.bracketData.bracket[self.bracketData.constants.FINAL_ID]
    )
    decorated[region.id].rounds = _map(
      region.rounds,
...
Validator.prototype.validatePicks = function (options) {
  ...
  _each(rounds, function (round, i) {
    ...
    var thisRoundPickedGames = _without(
      round,
      self.bracketData.constants.UNPICKED_MATCH
    )
    var nextRoundPickedGames = nextRound
      ? _without(nextRound, self.bracketData.constants.UNPICKED_MATCH)
      : []
    var nextRoundIsSubset =
      !lastItem && _subset(nextRoundPickedGames, thisRoundPickedGames)
    var winsInCorrect = _every(winsInThis, function (w) {
      return (
        w === null || self.bracketData.constants.BEST_OF_RANGE.indexOf(w) > -1
      )
    })
...
Validator.prototype.getRounds = function (options) {
  ...
  var retRounds = [
    regionName === this.bracketData.constants.FINAL_ID
      ? this.bracketData.constants.REGION_IDS
      : this.bracketData.order,
  ]
  var retWinsIn = [[]]
  ...
  while (length > 1) {
    length = length / 2
    var roundGames = verify(
      _map(rounds.splice(0, Math.floor(length)), checkVal),
      [this.bracketData.constants.UNPICKED_MATCH]
    )
...
Validator.prototype.picksToArray = function (picks, regionName) {
  ...
  var firstRoundLength =
    regionName === this.bracketData.constants.FINAL_ID
      ? this.bracketData.constants.REGION_COUNT
      : this.bracketData.constants.TEAMS_PER_REGION
  var replacement =
    "$" +
  ...
  var backref = function (start, stop, step) {
    ...
    return regExpJoiner(
      _map(_range(start, stop + 1, step), function (n) {
        return "\\" + n
      }),
      true
    )
  ...
  if (regionName === this.bracketData.constants.FINAL_ID) {
    ...
    for (i = 0; i < this.bracketData.constants.REGION_COUNT - 1; i++) {
      regExpGroups.push(
        regExpJoiner(seeds.slice(0, this.bracketData.constants.REGION_COUNT))
      )
...
Validator.prototype.validateFinal = function (finalPicks, validatedRounds) {
  ...
  for (var i = 0, m = validatedRounds.length; i < m; i++) {
    ...
    if (
      regionId !== this.bracketData.constants.FINAL_ID &&
      regionWinner === this.bracketData.constants.UNPICKED_MATCH &&
  ...
  var playingWrongSide =
    this.bracketData.bracket.regions[semifinal[0]].sameSideAs === semifinal[1]
