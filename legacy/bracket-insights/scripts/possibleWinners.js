...
module.exports = (o) => {
  ...
  const unpicked = (
    master.match(new RegExp(o.data.constants.UNPICKED_MATCH, "g")) || []
  ).length

  if (unpicked > 7) {
    throw new Error(
      "This script cant handle creating text descriptions for brackets with more than 7 games remaining"
    )
  }
  ...
  return _.chain(current)
    .reduce((acc, master) => {
      const scored = _.orderBy(
        o.scorer.score(o.scoring, { entry: o.entries, master }),
        "score",
        "desc"
      )
      const winningScore = scored[0].score
      const winners = _.filter(scored, { score: winningScore }).map(
        (winner) => {
          return {
