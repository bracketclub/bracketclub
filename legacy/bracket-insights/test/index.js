...
const exexScript = (index, args = {}) => (t, cb = _.noop) => {
  ...
  exec(
    `node index --script ${scriptNames[index]} ${stringifyArgs(scriptArgs)}`,
    (err, stdout, stderr) => {
      t.notOk(err)
      t.notOk(stderr)
      t.ok(stdout)
...
test("possible-winners down to elite 8", (t) => {
  exexScript("possible-winners", { master: -7 })(t, (err, { output }) => {
    const data = JSON.parse(output.split("Non-eliminated Entries")[0])
