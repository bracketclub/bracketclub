...
describe("Batch", function () {
  it("Should score multiple entries", function () {
    var lower = new BracketGenerator({
      winners: "lower",
      year: year,
      sport: sport,
    }).generate()
    var higher = new BracketGenerator({
      winners: "higher",
      year: year,
      sport: sport,
    }).generate()
    var master =
      "MW191213614102113610131010W185131114101515141011414S191246147159414741414E16851361472165147575FFWEE"
  ...
  it("Should score multiple entries and keep data", function () {
    ...
    var scorer = new BracketScorer({
      entry: [
        { name: "luke", bracket: lower },
        { name: "luke2", bracket: higher },
      ],
      master: master,
      sport: sport,
      year: year,
...
describe("Bracket Scorer", function () {
  it("Should return rounds", function () {
    var noUpsets = new BracketGenerator({
      winners: "lower",
      year: year,
      sport: sport,
    })
    var random = new BracketGenerator({
      winners: "lower",
      year: year,
      sport: sport,
    })
    ...
    var perf = new BracketGenerator({
      winners: "lower",
      year: year,
      sport: sport,
    }).generate()
    var perf2 = new BracketGenerator({
      winners: "higher",
      year: year,
      sport: sport,
    }).generate()
  ...
  it("Perfect score", function () {
    var entry = new BracketGenerator({
      winners: "lower",
      year: year,
      sport: sport,
    })
    var master = new BracketGenerator({
      winners: "lower",
      year: year,
      sport: sport,
    })
  ...
  it("Worst score", function () {
    ...
    var allUpsets = new BracketGenerator({
      winners: "higher",
      year: year,
      sport: sport,
    })
  ...
  it("Can be reset", function () {
    ...
    var s = scorer.score("standard")

    assert.equal(s, 0)

    s = scorer.standard({
      master: new BracketGenerator({
        winners: "higher",
        year: year,
        sport: sport,
      }).generate(),
  ...
  it("Possible points remaining: standard", function () {
    var s = new BracketScorer({
      entry:
        "S195463721437171E1812463721432424W19124637211237131MW181241131021432133FFSWS",
      master:
        "S19124113102141110XXXE1812463721437XXXW1812463721462XXXMW185411147284112XXXFFXXX",
      sport: sport,
      year: "2014",
    }).score(["standardPPR"])
  ...
  it("Ad hoc scoring systems", function () {
    ...
    var oneWrong = new BracketScorer({
    ...
    assert.throws(function () {
      s.score("notASystem")
    }, /no scoring system: notASystem/)
    assert.throws(function () {
      s.score("doesNotWork")
    }, /Cant do anything with scoring system: doesNotWork/)
  })
})
...
describe("Bracket Differ", function () {
  ...
  it("Check games: round 1 wrong", function () {
    ...
    assert.equal(
      true,
      _.every(_.map(res.MW.rounds[2], "eliminated"), function (i) {
        return i === true
      })
    )
    assert.equal(
      true,
      _.every(_.map(res.W.rounds[2], "eliminated"), function (i) {
        return i === true
      })
    )
    assert.equal(
      true,
      _.every(_.map(res.S.rounds[2], "eliminated"), function (i) {
        return i === true
      })
    )
    assert.equal(
      true,
      _.every(_.map(res.E.rounds[2], "eliminated"), function (i) {
        return i === true
      })
    )
    ...
    assert.equal(
      true,
      _.every(_.map(res.MW.rounds[3], "eliminated"), function (i) {
        return i === true
      })
    )
    assert.equal(
      true,
      _.every(_.map(res.W.rounds[3], "eliminated"), function (i) {
        return i === true
      })
    )
    assert.equal(
      true,
      _.every(_.map(res.S.rounds[3], "eliminated"), function (i) {
        return i === true
      })
    )
    assert.equal(
      true,
      _.every(_.map(res.E.rounds[3], "eliminated"), function (i) {
        return i === true
      })
    )
    ...
    assert.equal(
      true,
      _.every(_.map(res.MW.rounds[4], "eliminated"), function (i) {
        return i === true
      })
    )
    assert.equal(
      true,
      _.every(_.map(res.W.rounds[4], "eliminated"), function (i) {
        return i === true
      })
    )
    assert.equal(
      true,
      _.every(_.map(res.S.rounds[4], "eliminated"), function (i) {
        return i === true
      })
    )
    assert.equal(
      true,
      _.every(_.map(res.E.rounds[4], "eliminated"), function (i) {
        return i === true
      })
    )
  ...
  it("Check games: round 1 right", function () {
    ...
    assert.deepEqual(
      { fromRegion: "MW", seed: 1, name: "Louisville" },
      res.MW.rounds[1][0].shouldBe
    )
    assert.deepEqual(
      { fromRegion: "W", seed: 1, name: "Gonzaga" },
      res.W.rounds[1][0].shouldBe
    )
    assert.deepEqual(
      { fromRegion: "E", seed: 1, name: "Indiana" },
      res.E.rounds[1][0].shouldBe
    )
    assert.deepEqual(
      { fromRegion: "S", seed: 1, name: "Kansas" },
      res.S.rounds[1][0].shouldBe
    )
  })
  ...
  it("Check final four, ncg, champion: eliminated teams", function () {
    var user =
      "S195463721437171E1812463721432424W19124637211237131MW181241131021432133FFSMWMW"
    var master =
      "S191241131021X1110XXXE181246372X4X7XXXW181246372X4X2XXXMW185411147284X2XXXFFXXX"
    var res = new BracketScorer({
      entry: user,
      master: master,
      year: "2014",
