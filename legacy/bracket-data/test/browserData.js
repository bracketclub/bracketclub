...
describe("Bracket Data", function () {
  it("should be able to create multiple data objects", function () {
    ...
    var sRegionTeams = _.map(brackets, function (b) {
      return [b[0].bracket.regions.S.teams, b[1].bracket.regions.S.teams]
    })
    ...
    assert.equal(
      _.map(sRegionTeams, "0.0").join(),
      "Kentucky,Kansas,Florida,Duke"
    )
    assert.equal(
      _.map(sRegionTeams, "1.0").join(),
      "Kentucky,Kansas,Florida,Duke"
    )

    brackets.forEach(function (b) {
      assert.deepEqual(Object.keys(b[0]), [
        "bracket",
        "constants",
        "regex",
        "order",
